...
typedef enum comms_state_t {
    CommsState_Length, // Receive Length State
    CommsState_Data, // Receive Data State
    CommsState_CRC // Receive CRC Byte State
} comms_state_t;

comms_state_t state = CommsState_Length;
uint8_t data_byte_count = 0;

comms_packet_t temporary_packet = {.length= 0, .data = {0}, .crc = 0};
comms_packet_t retx_packet = {.length= 0, .data = {0}, .crc = 0}; // re-transmit packet
comms_packet_t ack_packet = {.length= 0, .data = {0}, .crc = 0}; // acknowledge packet
comms_packet_t last_transmitted_packet = {.length= 0, .data = {0}, .crc = 0};

comms_packet_t packet_buffer[PACKET_BUFFER_LENGTH];
uint32_t packet_read_index = 0;
uint32_t packet_write_index = 0;
uint32_t packet_buffer_mask = PACKET_BUFFER_LENGTH - 1;
